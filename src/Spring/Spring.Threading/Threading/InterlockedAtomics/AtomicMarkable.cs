...
namespace Spring.Threading.InterlockedAtomics
{
    ...
    public class AtomicMarkable<T> {
        ...
        private class ValueBooleanPair {
            ...
            internal readonly bool MarkBit;
        ...
        public virtual bool WeakCompareAndSet(T expectedValue, T newValue, bool expectedMark, bool newMark) {
            ...
            return AreEqual(expectedValue, current.Value) && expectedMark == current.MarkBit &&
                ((AreEqual(newValue, current.Value) && newMark == current.MarkBit) || _atomicReference.CompareAndSet(current, new ValueBooleanPair(newValue, newMark)));
        }
        ...
        public void SetNewAtomicValue(T newValue, bool newMark) {
            ValueBooleanPair current = Pair;
            if (!AreEqual(newValue, current.Value) || newMark != current.MarkBit)
        ...
        public bool AttemptMark(T expectedValue, bool newMark) {
            ...
            return AreEqual(expectedValue, current.Value) 
                && (newMark == current.MarkBit || _atomicReference.CompareAndSet(current, new ValueBooleanPair(expectedValue, newMark)));
        }
